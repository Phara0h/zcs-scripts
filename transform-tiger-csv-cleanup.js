...
const DATA_FILE_LOC = process.cwd() + "/dumps/streets-tiger.csv";
const fs = require("fs");
const fsproms = require("fs/promises");
const readline = require("readline");
var file_loc = `/dumps/streets-tiger_clean.csv`;
...
async function transformData() {
  ...
  rl.on("line", (line) => {
    ...
    var street_name = columns.shift().split(" ");
    var street_name_text = "";
    for (var i = 0; i < street_name.length; i++) {
      if (roads[street_name[i]]) {
        street_name_text +=
          roads[street_name[i]].full + (i + 1 < street_name.length ? " " : "");
      } else {
        street_name_text +=
          street_name[i] + (i + 1 < street_name.length ? " " : "");
      }
    }

    writeStream.write(street_name_text + "," + columns.join(",") + "\n");
    //console.log(columns);
